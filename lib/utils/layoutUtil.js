...
export function connectElements(source, target, layoutGrid) {
  ...
  if (dY === 0) {
    console.log(layoutGrid.getElementsInRange(source.gridPosition, target.gridPosition));
  ...
  if (dX === 0) {
    if (layoutGrid.getElementsInRange(source.gridPosition, target.gridPosition).length > 2) {
      ...
      return [
        ...
        getDockingPoint(targetMid, targetBounds, Math.sign(dY) ? 'b' : 't')
      ];
    } else {
...
// helpers /////
export function getBounds(element, row, col) {
